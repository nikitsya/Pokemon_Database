...
WHERE caught_id = p1_id
  AND trainer_id = t1_id;
SET
rows1 = ROW_COUNT();
...
WHERE caught_id = p2_id
  AND trainer_id = t2_id;
SET
rows2 = ROW_COUNT();

    -- Commit only if both Pokémon were transferred successfully.
    IF
rows1 = 1 AND rows2 = 1 THEN
        COMMIT;
