...
CREATE PROCEDURE AddPokemon(IN species_name VARCHAR (50), IN baseHP INT, IN baseAttack INT, IN baseDefense INT,
                            IN type1ID INT, IN type2ID INT, IN regionID INT, IN locationDescription VARCHAR (50),
                            IN minLevel INT, IN maxLevel INT)
BEGIN
...
new_trainer_id INT;
    DECLARE
hp_iv INT DEFAULT FLOOR(RAND() * 31);
    DECLARE
atk_iv INT DEFAULT FLOOR(RAND() * 31);
    DECLARE
def_iv INT DEFAULT FLOOR(RAND() * 31);
